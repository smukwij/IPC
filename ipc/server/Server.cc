...
void Server::shut_down(const std::string& addr)
{
    logging::log_info << "Server client close status: " <<strerror(close(_client_socket_fd)) << logging::end;
    logging::log_info << "Server close status: " <<strerror(close(_socket_fd)) << logging::end;
    unlink(addr.c_str());
}
