    ...
    : _socket_fd(-1)
{
    std::string client = "";
    std::string server = "";
    connection::Connection<connection::Socket> con = connection::create<connection::Socket>(client, server);
}
...
void Client::shut_down(const std::string& addr)
{
    ...
    logging::log_info << "Client close status: " <<strerror(close(_socket_fd)) << logging::end;
    unlink(addr.c_str());
}
